export default (editor, url) => {
    ...
    const modAIPromptHandler = () => {
        ...
        return modAI.ui.localChat({
            namespace: 'fredrtetinymce.modai',
            field: editor.id,
            key: editor.id,
            availableTypes: ['text', 'image'],
            context: selectedText,
            customCSS: editor.contentCSS,
            resource: fred.getConfig()._resource.id,
            image: {
                mediaSource: editor.targetElm.fredEl.options?.imageMediaSource || editor.targetElm.fredEl.options?.mediaSource
            },
            textActions: {
                insert: (msg, modal) => {
